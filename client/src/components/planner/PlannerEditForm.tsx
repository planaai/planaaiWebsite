                    ...
                    <div className={`grid grid-cols-1 sm:grid-cols-2 gap-6 transition-opacity ${editingPlan.targetWeaponStar === 0 ? 'opacity-40 pointer-events-none' : ''}`}>
                        <div>
                            ...
                            <div className="flex gap-1 items-center">
                                <input type="number" value={editingPlan.currentWeaponStar || 0} disabled className="w-12 sm:w-16 flex-shrink-0 bg-slate-100 border border-slate-200 rounded-lg px-2 py-1.5 text-center text-slate-400 cursor-not-allowed" />
                                ...
                                <div className="flex-1 flex items-stretch bg-white border-2 border-pink-200 focus-within:border-[var(--plana-primary)] rounded-lg overflow-hidden transition-colors">
                                    <input type="number" min={Math.max(1, editingPlan.currentWeaponStar || 1)} max="4" value={editingPlan.targetWeaponStar === 0 ? '' : (editingPlan.targetWeaponStar || '')} onChange={e => { const v = Math.max((parseInt(e.target.value) || 1), editingPlan.currentWeaponStar || 0); const maxLvl = getWeaponMaxLevel(v); setEditingPlan({...editingPlan, targetWeaponStar: v, targetWeaponLevel: Math.max(Math.min(editingPlan.targetWeaponLevel || 1, maxLvl), editingPlan.currentWeaponLevel || 1), targetStar: v >= 1 ? 5 : editingPlan.targetStar}); }} className="w-full bg-transparent px-2 py-1.5 text-center text-slate-800 focus:outline-none min-w-0" />
                        ...
                        <div>
                            ...
                            <div className="flex gap-1 items-center">
                                <input type="number" value={editingPlan.currentWeaponStar ? (editingPlan.currentWeaponLevel || 1) : ''} disabled className="w-12 sm:w-16 flex-shrink-0 bg-slate-100 border border-slate-200 rounded-lg px-2 py-1.5 text-center text-slate-400 cursor-not-allowed" />
                                <span className="text-slate-300 font-bold mx-1">➔</span>
                                <div className="flex-1 flex items-stretch bg-white border-2 border-pink-200 focus-within:border-[var(--plana-primary)] rounded-lg overflow-hidden transition-colors">
