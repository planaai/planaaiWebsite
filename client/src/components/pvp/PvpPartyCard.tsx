...
export function PvpPartyCard({ party: rawParty, masterData, onDelete, isDetail = false, bossName }: Props) {
  ...
  const party = {
    ...rawParty,
    party: {
      strikers: rawParty.party?.strikers || [],
      specials: rawParty.party?.specials || [],
    },
    tags: rawParty.tags || [],
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      const res = await api.post(`/pvp/parties/${party.id}/like`);
    ...
    <div className={
      ...
      <div className={`flex justify-between items-start ${isDetail ? 'border-b border-gray-100 pb-5 mb-2' : ''}`}>
        ...
        <div className="flex flex-col gap-2 items-end flex-shrink-0">
          ...
          {!isDetail && (
            <Link 
              href={`/tactics/pvp/detail?code=${party.shortCode || party.id}`}
        ...
        <div className="flex flex-col gap-4 mt-2">
          ...
          <div className="flex justify-between items-center pt-2">
            {canDelete && onDelete ? (
              <div className="flex items-center gap-2">
                <Link 
                  href={`/tactics/pvp/edit?id=${party.shortCode || party.id}`}
