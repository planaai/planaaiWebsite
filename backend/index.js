...
const uploadOcrError = multer({ dest: ocrErrorsDir, limits: { fileSize: FILE_SIZE_LIMIT } });

app.post('/api/archive/ocr-error-log', uploadOcrError.single('image'), (req, res) => {
  const { rawText, error } = req.body;
  const logData = {
  ...
  console.log('OCR Error Logged:', logData);
  fs.appendFileSync(path.join(ocrErrorsDir, 'error-log.jsonl'), JSON.stringify(logData) + '\n');
  res.json({ status: 'success' });
});
