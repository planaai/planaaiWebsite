...
1. **DB 작업 미완료 이슈**
   * 대상 학원: 산해경, 붉은겨울, SRT, 발키리, 아리우스, 하이렌더, 와일드헌트, 콜라보레이션
2. **이미지 리소스 미완료 이슈**
3. **일부 학생 추가 스킬 미구현**
   * **레이사(매지컬)**: 테르밋 핑크 전용 스킬
   * **호시노(무장)**: 탱커/딜러 모드 넣기
   * **아카리(새해)**: 급양추진차량 스킬
   * **히마리(무장)**: "바이크" 스킬
   * **이로하**: 토라마루 스킬
4. **선물 도감 오류**: 고급 선물 5개 등록 안됨
5. **모의 육성 오류**: 모의 스킬 육성에서 오파츠 이미지를 불러오지 못하는 이슈
6. **클라이언트 기능 오류**: plana.ai 클라이언트 스크린샷 기능 미작동
7. **회원가입 UI 오류**: 회원가입 입력란이 검은 색으로 되어 있는 이슈
8. **학생 등록 오류**: 컬렉션에서 학생을 수동 등록을 할 경우, 위의 X를 눌러 입력창을 나가지 못하는 이슈
9. **서버 동기화 오류**: 컬렉션에서 학생을 등록한 학생을 서버에 동기화 할때 서버에 전송을 실패하는 이
---
